...
def prepare_dataset(data='dev'):
  ...
  if data == 'dev':
    # load dev set (1K)
    filename = 'D:/data/Flickr_8k.devImages.txt'
    ...
    # descriptions
    train_descriptions = load_clean_descriptions('D:/caption/models/descriptions.txt', train)
    test_descriptions = load_clean_descriptions('D:/caption/models/descriptions.txt', test)
    print('Descriptions: train=%d, test=%d' % (len(train_descriptions), len(test_descriptions)))

    # photo features
    train_features = load_photo_features('D:/caption/models/features.pkl', train)
    test_features = load_photo_features('D:/caption/models/features.pkl', test)
    print('Photos: train=%d, test=%d' % (len(train_features), len(test_features)))

  elif data == 'train':
    # load training dataset (6K)
    filename = 'D:/data/Flickr_8k.trainImages.txt'
    ...
    # load test set
    filename = 'D:/data/Flickr_8k.testImages.txt'
    test = load_set(filename)
